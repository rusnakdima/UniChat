...
use serde::Serialize;
use tauri_shared::{log_error, log_info};
...
pub async fn kick_fetch_chatroom_id(
  ...
  validate_channel_slug(&channel_slug).map_err(|e| format!("Invalid channel slug: {}", e))?;
  if let Some(ref token) = access_token {
...
pub async fn kick_fetch_channel_info(
  ...
  let user_id = user.id.ok_or_else(|| "User ID not found")?;
  ...
  let channel_id = data
    ...
    .and_then(|c| c.id)
    .or(data.id)
    .ok_or_else(|| "Channel ID not found in response")?;
