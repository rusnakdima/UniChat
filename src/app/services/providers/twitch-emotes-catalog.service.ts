...
export class TwitchEmotesCatalogService {
  ...
  getEmotes(): TwitchChannelEmote[] {
    return Array.from(this._emotes.values());
  }
  getEmoteByCode(code: string): TwitchChannelEmote | null {
    for (const emote of this._emotes.values()) {
      if (emote.code === code) return emote;
    }
    return null;
  }
  fetchTwitchChannelEmotes(channelId: string): Promise<TwitchChannelEmote[]> {
